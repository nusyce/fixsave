...
trait JobTrait
{
    ...
    public function storeFrontJob(Request $request)
    {
        ...
        $job->branche = $request->input('branche');
        $job->beruf = $request->input('beruf');
        $job->bereich = $request->input('bereich');
        $job->zeit = $request->input('zeit');
        ...
        $job->land = $request->input('land');
        $job->bundesland = $request->input('bundesland');
        $job->plz = $request->input('plz');
        $job->stadt = $request->input('stadt');
        $job->arbeitsort = $request->input('arbeitsort');
        $job->deutschlevel = $request->input('deutschlevel');
        $job->englischlevel = $request->input('englischlevel');
        $job->sonstige = $request->input('sonstige');
        $job->fuhrerschein = $request->input('fuhrerschein');
        $job->unterkunft = $request->input('unterkunft');
    ...
    public function  ajaxWahle(Request $request){
        $formated = array();
        $rest =  Wahle::where('name','like', '%' . $request->get('query') . '%')->get()->toArray();
        foreach ($rest as $rests) {
            array_push($formated, array('value'=>$rests['name'],'data'=>$rests['name']));
        }
        $obj = new \stdClass();
        $obj->suggestions = $formated;
